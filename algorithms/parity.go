...
// PointInPolygon determines if a point is inside a polygon using the even-odd rule
...
// ParityFillPolygonWithImage fills a polygon with an image texture using point-in-polygon tests
func ParityFillPolygonWithImage(canvas [][]color.Color, vertices []Point, fillImage [][]color.Color) {
	...
	// Scan each pixel in the bounding box and fill if inside polygon
	for y := minY; y <= maxY; y++ {
		for x := minX; x <= maxX; x++ {
			if PointInPolygon(Point{X: x, Y: y}, vertices) {
